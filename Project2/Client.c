...
void *worker_handler()
{
    ...
    while (is_running || theQueue->num_jobs > 0)
    {
        ...
        if ((strcmp(req_str[0], "CHECK") == 0) && (num_args == 2))
        {
            ...
            pthread_mutex_lock(&accounts[check_accnt].lock);
            check_amnt = read_account(check_accnt);
            gettimeofday(&curr_req->endtime, NULL);
            pthread_mutex_unlock(&accounts[check_accnt].lock);
            flockfile(out_fp);
        ...
        else if ((strcmp(req_str[0], "TRANS") == 0) && num_args > 2)
        {
            ...
            {
                // //For Testing
                // printf("Account %d was locked\n", trans_accnt[i]);
                pthread_mutex_lock(&accounts[trans_accnt[i]].lock);
            }
            ...
            // Unlock the accounts
            for(int i = 0; i < num_of_accounts; i++){
                pthread_mutex_unlock(&accounts[trans_accnt[i]].lock);
                // //For Testing
                // printf("Account %d was unlocked\n", trans_accnt[i]);
            }
            
            // //For Testing
            // printf("TRANS finished\n");
        }
        else
        {
            // TODO
